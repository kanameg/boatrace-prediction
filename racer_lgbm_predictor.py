...
def train_model(X_train, y_train, feature_names=None):
    ...
    if feature_names is not None:
        ...
        # 重要度の正規化（合計を1にする）
        importance_df["重要度_正規化"] = (
            importance_df["重要度"] / importance_df["重要度"].sum()
        )

        for i, row in importance_df.iterrows():
            print(
                f"{row['特徴量']:<20}: {row['重要度']:.0f} ({row['重要度_正規化']*100:.2f}%)"
            )
        print()
...
def load_model(file_path="model/lightgbm_model.pkl"):
    """学習済みモデルを読み込む"""
    try:
...
def predict_results(model, test_data, config=None):
    ...
    # 予測実行（LightGBMの場合は確率を取得してから二値化）
    predictions_proba = model.predict(X_test, num_iteration=model.best_iteration)
