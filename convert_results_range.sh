...
#
# 使用方法: ./convert_results_range.sh START_YYYY START_MM START_DD END_YYYY END_MM END_DD
#
...
show_usage() {
    ...
    echo ""
    echo "例: $0 2025 7 1 2025 7 9  # 2025年7月1日から9日まで競走結果データを変換"
    exit 1
}
...
while [ "$CURRENT_EPOCH" -le "$END_EPOCH" ]; do
    ...
    # convert_result.pyを実行
    if python3 convert_result.py "$YEAR" "$MONTH" "$DAY"; then
        echo "  → 成功"
        SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
    else
        echo "  → エラー: ${YEAR}年${MONTH}月${DAY}日の変換に失敗しました"
        ERROR_COUNT=$((ERROR_COUNT + 1))
        error_exit "各日付の変換に失敗しました: ${YEAR}年${MONTH}月${DAY}日"
    fi
